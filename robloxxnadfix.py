...
async def resolve_deal_dispute(deal_id: int, winner_id: int, admin_id: int, amount: float):
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        # В текущей реализации P2P-сделка это *прямой* платеж продавцу. 
        # Решение спора: если выигрывает продавец, он ничего не получает, так как деньги у него уже есть. Если выигрывает покупатель, мы *штрафуем* продавца на эту сумму и возвращаем покупателю.
        
        # Для простоты: Администратор принимает решение, кому зачисляются средства/робуксы.
        ...
        # Если выигрывает ПРОДАВЕЦ: статус 'completed'. Ничего не делаем с балансом, т.к. деньги у продавца.
        # Если выигрывает ПОКУПАТЕЛЬ: статус 'dispute_buyer_won'. Списываем деньги с баланса продавца, если они у него есть, и возвращаем покупателю (если он платил через YooKassa, то надо делать refund).
        # Для текущего кода, где оплата YooKassa идет *напрямую* продавцу:
        # Решение спора должно быть ручным: Админ должен решить, делать ли Refund через YooKassa (если возможно) или списать с баланса продавца и вернуть покупателю.
        
        # Упрощаем: Переводим в статус 'resolved' и админ выполняет финансовые операции вручную или через отдельный интерфейс.
...
async def support_handler(callback: types.CallbackQuery, bot: Bot):
    ...
    await callback.message.edit_text(
        f"✉️ **Написать в поддержку**\n\nДля связи с администратором, пожалуйста, перейдите по ссылке: [Поддержка](tg://user?id={SUPPORT_ADMIN_ID})\n"
        "Ваш ID будет автоматически передан администратору.",
...
async def profile_referral_cb(call: types.CallbackQuery):
    ...
    # Экранируем данные для MarkdownV2
    ...
    text = (
        ...
        f"Приглашайте друзей и получайте **{bonus_esc} ₽** на баланс за каждого нового пользователя\\!\n\n"
        f"📊 **Ваша статистика:**\n"
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        # Кнопка "Поделиться" открывает нативное меню Telegram для отправки контакта
        [InlineKeyboardButton(text="📤 Отправить другу", url=f"https://t.me/share/url?url={ref_link}&text=Заходи%20и%20покупай%20Robux%20выгодно!")],
...
async def profile_tx_cb(call: types.CallbackQuery):
    ...
    # 1. Заголовок
    # Используем MarkdownV2, поэтому * нужно заменить на ** для жирного шрифта
    ...
    if not transactions:
        ...
        for event_type, details, created_at in transactions:
            ...
            if details:
                try:
                    # Убедитесь, что json.loads() доступен (импортирован)
            ...
            # 💡 ИСПРАВЛЕНИЕ 1: Экранируем дату, чтобы точки были безопасны.
            tx_date_esc = escape_markdown_v2(tx_date) 

            # Эмодзи статуса (предполагается, что функция status_icon() существует)
            icon = status_icon(event_type)

            # 💡 ИСПРАВЛЕНИЕ 2: Используем экранированную дату (tx_date_esc)
            # 💡 ИСПРАВЛЕНИЕ 3: Используем \\- для избежания SyntaxWarning
            line = f"{icon} \\-\\- {tx_date_esc}: "

            # Типы событий (Форматируем для MarkdownV2 сразу)
            if event_type == "BUY_ROBUX":
                # Предполагается, что format_number возвращает строку с числами, которые могут содержать точки/запятые
                ...
                # Экранируем минус, чтобы он не воспринимался как начало жирного текста
                line += f"**ВЫВОД RUB**: \\-{amount} ₽"

            elif event_type == "REFILL_BALANCE":
                amount = escape_markdown_v2(format_number(readable_details.get("rub_amount", "N/A")))
                # Экранируем плюс, который нужно было бы экранировать, если бы он был в начале строки, но здесь он безопасен
                ...
                # Если тип не распознан, экранируем все детали
                details_esc = escape_markdown_v2(str(readable_details.get("raw", details)))
                line += f"{escape_markdown_v2(event_type)}: {details_esc}"

            # 💡 УБРАНО: escape_markdown_v2(line) - теперь форматирование (**) обрабатывается MarkdownV2, 
            # а все опасные символы внутри переменных экранированы.
            text.append(line)

    # Клавиатура с кнопкой "Назад"
    # Предполагается, что inline_profile_menu(uid) существует
...
async def withdraw_start(call: types.CallbackQuery, state: FSMContext):
    ...
    if balance < min_withdraw:
        return await call.message.edit_text(
            f"❌ **Вывод средств**\n\nМинимальная сумма вывода: **{min_withdraw:,.2f} ₽**\n"
